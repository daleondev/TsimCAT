...
namespace
{
    ...
    {
        uint16_t ns;
        std::variant<uint32_t, std::string> id;
    };

    static auto strToNode(std::string_view str) -> std::optional<UaNode>
    {
        UaNode node{};
        ...
        if (str.starts_with("ns=")) {
            ...
            auto semiPos = str.find(';');
            if (semiPos == std::string_view::npos) {
                return std::nullopt;
            }

            auto [ptr, ec] = std::from_chars(str.data(), str.data() + semiPos, node.ns);
            if (ec != std::errc()) {
                return std::nullopt;
            }

            str.remove_prefix(semiPos + 1);
        }
    ...
    static auto nodeToNative(UaNode node) -> UA_NodeId
    {
        if (node.id.index() == 0) {
            return UA_NODEID_NUMERIC(node.ns, std::get<0>(node.id));
        }
        else {
            return UA_NODEID_STRING(node.ns, const_cast<char*>(std::get<1>(node.id).c_str()));
        }
    }

    // clang-format off
    using UaStatusType = UA_StatusCode;
    enum class UaStatus : UaStatusType
    {
        InfoTypeDataValue = 0x00000400,          /**< The StatusCode contains a DataValue. */
        InfoBitsOverflow = 0x00000080,           /**< The StatusCode contains an overflow bit. */
        ...
        BadLicenseLimitsExceeded = 0x810F0000,   /**< The server has limits on number of allowed operations / object. */
        ...
        BadContinuationPointInvalid = 0x804A0000, /**< The continuation point provided is no longer valid. */
        ...
        BadNodeIdRejected = 0x805D0000,          /**< The requested node id was rejected because it was either invalid or server does not allow node ids to be specified by the client. */
        ...
        BadWriteNotSupported = 0x80730000,       /**< The server does not support writing the combination of values. */
        ...
        BadTimestampNotSupported = 0x80A10000,   /**< The client requested history using a timestamp format the server does not support. */
        ...
        BadDominantValueChanged = 0x80E10000,    /**< The related EngineeringUnit has been changed but this change has not been applied to the device. */
        ...
        GoodEditedDependentValueChanged = 0x01160000, /**< It is delivered with a dominant Variable value when a dependent Variable has changed but the change has not been applied. */
        GoodEditedDominantValueChanged = 0x01170000, /**< It is delivered with a dependent Variable value when a dominant Variable has changed but the change has not been applied. */
        GoodEditedDominantValueChangedDependentValueChanged = 0x01180000, /**< It is delivered with a dependent Variable value when a dominant or dependent Variable has changed but change has not been applied. */
        BadEditedOutOfRange = 0x81190000,        /**< It is delivered with a Variable value when Variable has changed but the value is not legal. */
        BadInitialValueOutOfRange = 0x811A0000,  /**< It is delivered with a Variable value when a source Variable has changed but the value is not legal. */
        BadOutOfRangeDominantValueChanged = 0x811B0000, /**< It is delivered with a dependent Variable value when a dominant Variable has changed and the value is not legal. */
        BadEditedOutOfRangeDominantValueChanged = 0x811C0000, /**< It is delivered with a dependent Variable value when a dominant Variable has change. */
        BadOutOfRangeDominantValueChangedDependentValueChanged = 0x811D0000, /**< It is delivered with a dependent Variable value when a dominant or dependent Variable has changed and the value is not legal. */
        BadEditedOutOfRangeDominantValueChangedDependentValueChanged = 0x811E0000, /**< It is delivered with a dependent Variable value when a dominant or dependent Variable has change. */
    ...
    {
        static UaStatusCategory instance;
        return instance;
    }

    std::error_code make_error_code(UaStatus e)
    {
