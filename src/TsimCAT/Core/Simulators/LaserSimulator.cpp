...
namespace core::sim
{
                ...
                while (m_running) {
                    ...
                    while (m_running && m_link->status() == link::Status::Connected) {
                        ...
                        bool done;
                        {
                            ...
                            if (done) m_laserDonePending = false;
                        }
    ...
    auto LaserSimulator::handleCommand(std::string_view cmd) -> coro::Task<void>
    {
        ...
        if (cmd == "[get state]") {
            ...
            if (state() == State::Idle) co_await transitionTo(State::Ready);
        } else if (cmd == "[close shutter]") {
            if (state() != State::Error) co_await transitionTo(State::Idle);
        } else if (cmd == "[pilot on]") {
            if (state() == State::Ready) co_await transitionTo(State::Pilot);
        } else if (cmd == "[pilot off]") {
            if (state() == State::Pilot) co_await transitionTo(State::Ready);
            ...
            if (state() == State::Done) co_await transitionTo(State::Idle);
    ...
    auto LaserSimulator::transitionTo(State newState) -> coro::Task<void>
    {
        ...
        switch (newState) {
            case State::Ready: co_await sendStatus("[ready]"); break;
            case State::Pilot: co_await sendStatus("[pilot busy]"); break;
            case State::Laser: co_await sendStatus("[laser busy]"); break;
            case State::Idle:  co_await sendStatus("[state <idle>]"); break;
            default: break;
        }
        co_return;
    }
